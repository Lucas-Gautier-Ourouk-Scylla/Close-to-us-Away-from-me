...
for (i in c(1:100)) {
  dat <- sim_design(within = list(vars = c("prefproxemique", "SocialClass")),
                ...
                r = 0.15, plot = FALSE, n = 400
                )
  dat$trial <- 1
  
   dat_Final <- dat
  for (c in c(2:120)) {
    dat_2 <- dat %>% mutate(prefproxemique = rnorm_pre(prefproxemique, r = 0.7),
                            trial = c)
...
for (i in c(1:100)) {
  dat <- sim_design(within = list(vars = c("prefproxemique")),
                    between = (couleur = c("green", "orange")),
                mu = list(prefproxemique = 0, SocialClass = 0),
                sd = list(prefproxemique = 1, SocialClass = 1),
                r = 0.15, plot = FALSE, n = 400
                )
...
```{r}
between <- list(couleur = c(vert = "vert", 
                        orange = "orange"))
